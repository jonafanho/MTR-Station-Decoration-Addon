...
public class RenderYamanoteRailwaySign<T extends BlockYamanoteRailwaySign.BlockYamanoteRailwaySignEntity> extends BlockEntityRenderer<T> implements IBlock, IGui, IDrawing {
    ...
    public void render(T entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        final BlockYamanoteRailwaySign block = (BlockYamanoteRailwaySign) state.getBlock().data;
        if (entity.getSignIds().length != block.length) {
            return;
        }
        final Direction facing = IBlock.getStatePropertySafe(state, BlockYamanoteRailwaySign.FACING);
    ...
    public static void drawSign(GraphicsHolder graphicsHolder, @Nullable StoredMatrixTransformations storedMatrixTransformations, BlockPos pos, String signId, float x, float y, float size, float maxWidthLeft, float maxWidthRight, LongAVLTreeSet selectedIds, Direction facing, int backgroundColor, RenderYamanoteRailwaySign.DrawTexture drawTexture) {
        ...
        final float signSize = (sign.getSmall() ? BlockYamanoteRailwaySign.SMALL_SIGN_PERCENTAGE : 1) * size;
        ...
        if (storedMatrixTransformations != null && isExit) {
            ...
            final ObjectArrayList<DynamicTextureCache.DynamicResource> resourceLocationDataList = new ObjectArrayList<>();
