...
public class YamanoteRailwaySignScreen extends ScreenExtension implements IGui {
    ...
    public YamanoteRailwaySignScreen(BlockPos signPos) {
        ...
        if (world != null) {
            ...
            signIds = ((BlockYamanoteRailwaySign.BlockYamanoteRailwaySignEntity) entity.data).getSignIds();
            selectedIds = ((BlockYamanoteRailwaySign.BlockYamanoteRailwaySignEntity) entity.data).getSelectedIds();
            final Block block = world.getBlockState(signPos).getBlock();
            if (block.data instanceof BlockYamanoteRailwaySign) {
                length = ((BlockYamanoteRailwaySign) block.data).length;
            } else {
                length = 0;
            }
        } else {
    ...
    public void render(GraphicsHolder graphicsHolder, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < signIds.length; i++) {
            if (signIds[i] != null) {
                RenderYamanoteRailwaySign.drawSign(graphicsHolder, null, signPos, signIds[i], (width - SIGN_SIZE * length) / 2F + i * SIGN_SIZE, 0, SIGN_SIZE, RenderYamanoteRailwaySign.getMaxWidth(signIds, i, false), RenderYamanoteRailwaySign.getMaxWidth(signIds, i, true), selectedIds, Direction.UP, 0, (textureId, x, y, size, flipTexture) -> {
        ...
        if (editingIndex >= 0) {
            ...
            loopSigns((index, x, y, isBig) -> {
                ...
                final SignResource sign = RenderYamanoteRailwaySign.getSign(signId);
                if (sign != null) {
                    final boolean moveRight = sign.hasCustomText && sign.getFlipCustomText();
                    RenderYamanoteRailwaySign.drawSign(graphicsHolder, null, signPos, signId, (isBig ? xOffsetBig : xOffsetSmall) + x + (moveRight ? SIGN_BUTTON_SIZE * 2 : 0), BUTTON_Y_START + y, SIGN_BUTTON_SIZE, 2, 2, selectedIds, Direction.UP, 0, (textureId, x1, y1, size, flipTexture) -> {
                        final GuiDrawing guiDrawing = new GuiDrawing(graphicsHolder);
    ...
    public void onClose2() {
        super.onClose2();
        InitClient.REGISTRY_CLIENT.sendPacketToServer(new MSDPacketUpdateYamanoteRailwaySignConfig(signPos, selectedIds, signIds));
    }
    ...
    private int loopSigns(YamanoteRailwaySignScreen.LoopSignsCallback loopSignsCallback, boolean ignorePage) {
        ...
        for (int i = 0; i < allSignIds.size(); i++) {
            final SignResource sign = RenderYamanoteRailwaySign.getSign(allSignIds.get(i));
